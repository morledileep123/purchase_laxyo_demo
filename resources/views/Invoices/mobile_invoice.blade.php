...
<body>
<div id="background">
    <p id="bg-text">Laxyo Energy LTD</p>
</div>
    ...
    <div class="invoice-box">
        <table cellpadding="0" cellspacing="0">
                    ...
                    <table>
                        <tr>
                            ...
                            <td style="text-align: right">
                                ...
                                Balance Due: ${{($invoice->balance)}}<br>
                                <?php if ($invoice->deposit_amount > 0 && $invoice->payments->sum('payment_amount') < $invoice->deposit_amount)echo "Partial Due: $". ($invoice->deposit_amount);?>
                            </td>
                        </tr>
                    </table>
                </td>
            ...
            <tr class="total" style="text-align: left">
                ...
                <td></td>
                <td>Subtotal<br>
                    <?php if ($invoice->client->tax_rate > 0 ) echo "Sales Tax (". ($invoice->client->tax_rate) . "%)<br>"; ?>
                ...
                <td style="text-align: right">
                    ${{($invoice->products->sum('line_total'))}}<br>
                    <?php if ($invoice->client->tax_rate >0) echo "$" .($invoice->products->sum('line_total')  * ((100+$invoice->client->tax_rate) / 100)-$invoice->products->sum('line_total')) . "<br>";?>
                    <?php if ($invoice->discount > 0 && $invoice->discount_type  == 'Amount') echo "($" . ($invoice->discount).")<br>"; ?>
                    <?php if ($invoice->discount > 0 && $invoice->discount_type  == 'Percent') echo "($".($invoice->products->sum('line_total') * (($invoice->discount) / 100)).")<br>"; ?>
                    (${{($invoice->payments->sum('payment_amount'))}})<br>
                    ${{($invoice->balance)}}<br>
                    <?php if ($invoice->deposit_amount > 0 && $invoice->payments->sum('payment_amount') < $invoice->deposit_amount) echo "$".($invoice->deposit_amount);?>
                </td>
            </tr>
        </table>
        <hr>
