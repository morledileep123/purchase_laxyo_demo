  ...
  <body>
    ...
    <div class="wrapper">
      ...
      <nav class="navbar navbar-light bg-light justify-content-between" style="background-color: #fff;">
        <div class="container">
          <button type="button" class="btn btn-success" data-toggle="modal" data-target="#SendGRRtoAdminModal" style="color:white">
            Send to Super Admin
          </button>
          {{-- <a href="{{url('sendsuperadmin',$data->id)}}" type="button" class="btn btn-success send_purchase_order" style="color:white">Send to Super Admin</a> --}}
          <a href="/">
        ...
        <div class="form-inline">
          <div class="btn-group">
            ...
            <div class="dropdown-menu">  
              ...
              <a class="dropdown-item" href="{{url('invoicedownloadadmin',$data->id )}}"><i class="fa fa-file-pdf" aria-hidden="true"></i> PDF Download</a>                       
            </div>
                        ...
                        <table class="table" border="2">
                          ...
                          <tbody>
                            ...
                            <tr>
                              <td>{{$i++}}</td>
                              <td>{{$row->item_name}}</td>
                              {{-- <td data-toggle="modal" data-target=".bd-example-modal-sm"><i class="fas fa-edit">edit</i></td>   --}}

                              @if($row->po_qty == null)
                                <td><button class="po_qty_insert" value="{{$row->id}}"><i class="fa-solid fa-bag-shopping"></i></button></td>  
                              @else
                                <td>{{$row->po_qty}}</td>  
                              @endif
...
<!-- Modal Comment -->
<div class="modal fade" id="SendGRRtoAdminModal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      <div class="modal-header">
        <h6 class="modal-title" id="exampleModalLabel">Comments</h6>
        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
          <span aria-hidden="true">&times;</span>
        </button>
      </div>
      <form action="{{url('sendsuperadmin')}}" method="POST" enctype="multipart/form-data">
      @csrf
        <div class="modal-body">
          <input type="hidden" name="team_id" value="{{$data->team_id}}">
          <input type="hidden" name="id" value="{{$data->id}}">
          <textarea class="form-control" name="admin_comment" rows="4%"></textarea>           
        </div>
        <div class="modal-footer">
          <button class="btn btn-success btn-sm" name="submit" value="submit">Send </button>
...
{{-- Modal PO QTY Update  --}}
<div class="modal fade bd-example-modal-sm" id="po_quantity_update" tabindex="-1" role="dialog" aria-labelledby="mySmallModalLabel" aria-hidden="true">
  <div class="modal-dialog modal-sm">
    <div class="modal-content">
      <div class="modal-header">
        <h6 class="modal-title" id="exampleModalLabel">PO Quantity</h6>
      </div>
      <form action="{{url('admin_send_po_qty')}}" method="POST" enctype="multipart/form-data">
      @csrf
        <div class="modal-body">
          <input type="hidden" name="id" id="id">
          <input type="text" name="po_qty" class="form-control" placeholder="Quantity">          
        </div>
        <div class="modal-footer">
          <button class="btn btn-success btn-sm" name="submit" value="submit">Update</button>
        </div>
      </form>
    </div>
  </div>
...
<script type="text/javascript">
 $(document).ready(function(){
    ...
    //  PO quantity
    $('.po_qty_insert').click(function(e){
      e.preventDefault();
      var id = $(this).val();
      
      $('#id').val(id);

      $('#po_quantity_update').modal('show');
